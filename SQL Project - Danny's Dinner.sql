...
--Selecting all the data in the tables
SELECT *
FROM sales;
...
SELECT TOP(1)
	s.product_id, m.product_name, COUNT(*) purchase_count
FROM sales s
JOIN menu m
ON s.product_id = m.product_id
...
FROM (
	SELECT s.customer_id, m.product_name, COUNT(*) order_count,
		RANK() OVER (PARTITION BY s.customer_id ORDER BY COUNT(*) DESC) rank_number
	FROM sales s
	JOIN menu m
	ON s.product_id = m.product_id
	GROUP BY s.customer_id, m.product_name) t1
WHERE rank_number = 1;
...
FROM (
	SELECT s.customer_id, m.product_name, s.order_date, me.join_date,
		RANK() OVER (PARTITION BY s.customer_id ORDER BY s.order_date) rank_number
	FROM sales s
	JOIN members me
	ON s.customer_id = me.customer_id
	JOIN menu m
	ON s.product_id = m.product_id
	WHERE s.order_date > me.join_date) t1
WHERE rank_number = 1;
...
FROM (
	SELECT s.customer_id, m.product_name, s.order_date, me.join_date,
		RANK() OVER (PARTITION BY s.customer_id ORDER BY s.order_date DESC) rank_number
	FROM sales s
	JOIN members me
	ON s.customer_id = me.customer_id
	JOIN menu m
...
SELECT s.customer_id, COUNT(order_date) item_count, SUM(price) amount_spent
FROM sales s
JOIN members me
ON s.customer_id = me.customer_id
JOIN menu m
...
SELECT customer_id, SUM(point_calc) customer_point
FROM (
		SELECT customer_id, product_name, price,
			   CASE WHEN product_name = 'sushi' THEN price * 20 
				ELSE price * 10 END AS point_calc
		FROM sales s
		JOIN menu m
...
FROM (
	SELECT s.customer_id, m.product_name, m.price, s.order_date,
			   CASE WHEN order_date >= join_date THEN price * 20 END AS point_calc
	FROM sales s
	JOIN members me
	ON s.customer_id = me.customer_id
	JOIN menu m
	ON s.product_id = m.product_id
	WHERE MONTH(s.order_date) = 1) t1
GROUP BY customer_id;
...
--Selecting all the data from joint_table
